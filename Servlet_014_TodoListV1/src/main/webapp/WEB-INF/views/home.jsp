...
<meta charset="UTF-8">
<!-- media query
반응형 web에서 사용하는 CSS 기법을 사용하려면 아래 viewport 코드를 사용해야한다 -->
...
table.td_list { /* 아래 두 코드는 table의 기본으로 값임 */
...
table.td_list td.doit {
	...
	/* 
	width : 무고건 폭을 지정하는 것
	max-width : 포함된 tex(content) 길이에 따라
					조절을 하되 최소 크기만 제한
	*/
	/* 
	  td의 width 보다 큰 text를 말줄임표로 표헌하기
	  
	  td 3인데
	  첫번째 세번째 td 20%로 설정했기 때문에
	  여기 td는 전체 table width의 60%가 될것이고,
	  이 상태에서 max-width를 0으로 지정하고,
	  ellipsis 설정을 해주면 된다
	  
	  말 줄임을 사용할때는 기본 아래 4줄의 코드를 사용해야한다
	*/
	max-width: 0;
...
@media screen and (max-width: 800px) {
	h1, form.doit, table.td_list{
	/* 반응형 화면으로 설계할때 width는 px이 아닌 %로 설정하는 것이 좋음*/
	width: 95%;
	margin: 5px auto;
...
<script>
	...
	//	화면에 DOM 오소가 모두 완전하게 나타나면 내부의 코드를 실행하라
	document.addEventListener("DOMContentLoaded", ()=>{
		document.querySelector("table.td_list")
		// ()=> {  } : 화살표 함수
		// fumction() {  } 를 사용하는 것과 같음
...
<body>
	...
	<h1>TO DO LIST</h1>
		...
		<%--
		form tag의 action 속성
		form tag의 action 속성은 담긴 데이터를 submti할때(서버로 전송할때)
		어떤 uri path를 통해서 서버에 보낼것인가 지정하는 것
		
		만약 이 action 속성을 지정하지 않으면
		 현재 파일(home.jsp)을 보여줄대 사용한 uri 주소가 자동으로 설정이 된다
