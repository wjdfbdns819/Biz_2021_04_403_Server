...
public class TodoServiceImplV1 implements TodoService{
		...
		protected List<Map<String, Object>> select(ResultSet rSet) throws SQLException {
			...
			while(rSet.next()) {
				...
				for(int index = 0; index < columns ; index++) {
					...
					// 메타 데이터에서 index 위치의 칼럼명 getter
					// index 값은 1부터 시작한다
					String colName = md.getColumnName(index + 1);
					
					// ResultSet으로 부터 index 위의 칼럼에 
					// 저장된 실제 데이터 getter
	...
	private void viewSelect(ResultSet rSet) throws SQLException {
		 ...
		 *  실제 데이터와 메타 데이터가 모두 포함되어 있다
		 *  rSet 객체의 next() method를 한번 호출하면
		 *  결과 table의 첫번째 레콛를 읽을 수 있도록
		 *  준비 해준다
		 * 이어서 next() method를 계속해서 호출하면 
		 * 	 한줄씩 순서대로 읽을 수 있도록 준비해준다
		 * 만약 더이상 읽을 데이터가 없으면 false를 return 한다
